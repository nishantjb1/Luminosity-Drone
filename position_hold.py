...
class swift():
	...
	def __init__(self):
		...
		self.alt_error = 0.0 #variable for calculating error ie setpoint - curr position
		...
		self.sample_time = 1/30 # in seconds (write acc to your pid controller)
		...
		self.command_pub = rospy.Publisher('/drone_command', swift_msgs, queue_size=10)
		...
		self.pid_error_pub = rospy.Publisher('/pid_error', Float64, queue_size=10)
	...
	#	8. Add error_sum
		...
		self.alt_error = - (self.setpoint[2] - self.drone_position[2]) #error for z axis
		
		self.cmd.rcThrottle = int(1550 + (self.alt_error *(self.Kp[2])) + ((self.alt_error - self.prev_alt_error)*(self.Ki[2])) + ((self.sum_alt_error)*(self.Kd[2])))
		self.cmd.rcPitch = int(1500 + (self.pitch_error * (self.Kp[1])) + ((self.pitch_error - self.prev_pitch_error)*(self.Ki[1])) + ((self.sum_pitch_error)*(self.Kd[1])))
		self.cmd.rcRoll = int(1500 + (self.roll_error * (self.Kp[0])) + ((self.roll_error - self.prev_roll_error)*(self.Ki[0])) + ((self.sum_roll_error)*(self.Kd[0])))
